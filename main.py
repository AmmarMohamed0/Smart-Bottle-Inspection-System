...
# def save_all_response_to_file(track_id, response):
...
def save_response_to_file(track_id, response, crop_filename):
    """Save the analysis response to a separate .text file for each bottle."""
    timestamp = time.strftime("%Y-%m-%d %H:%M:%S")
...
def process_crop_image(current_image, track_id, crop_filename):
    ...
    # Save the analysis response to a separate file for each bottle
